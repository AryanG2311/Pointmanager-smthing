...
const AdminDashboard = () => {
  ...
  const fetchLeaderboard = async () => {
    try {
      const response = await fetch(`${backendHost}/api/leaderboard/admin`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${backendHost}/newuser`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdatePoints = async (userId) => {
    try {
      ...
      const response = await fetch(`${backendHost}/api/users/${userId}/points`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
